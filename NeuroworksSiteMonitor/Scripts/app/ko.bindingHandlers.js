...
function ($, ko) {
    ...
    ko.bindingHandlers.checkboxImage = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
            var $el = $(element),
                //MIKE: for call in  sessions.cshtml returns {checked: sessionFilter.favoriteOnly}
                settings = valueAccessor(); 

            $el.addClass('checkbox');

            $el.click(function () {
                if (settings.checked) {//MIKE: if settings.checked is defined, not the value of settins.checked()!!!
        ...
        update: function (element, valueAccessor) { 
            var $el = $(element),
                settings = valueAccessor(),
                //MIKE: looks like a subtle bug in the usage of unwrap, original code is commented
